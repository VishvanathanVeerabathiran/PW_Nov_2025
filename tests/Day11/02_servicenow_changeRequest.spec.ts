...
test.describe.serial(`Service Now`, async () => {
    ...
    test(`Fetch Newly created change request in service now using playwright API `, async ({ request }) => {
        const fetchCRResponse = await request.get(`https://dev291487.service-now.com/api/now/table/change_request?sysparm_limit=1`,
            {
                headers: { "Authorization": "Basic YWRtaW46QjdoL1E3T3dqYy9C", "Content-Type": "application/json", }, ignoreHTTPSErrors: true
            });
        ...
        crsystemId = fetchCRResponseBody.result[0].sys_id;
        changeRequestID = fetchCRResponseBody.result[0].number;
        console.log(`System id for change request is : ${crsystemId}`);
        console.log(`Change Request no : ${changeRequestID}`);
        //Get sys id from response body  // systemId= createResponseBody.result.sys_id;  // console.log(`System id : ${systemId}`);        
    })
